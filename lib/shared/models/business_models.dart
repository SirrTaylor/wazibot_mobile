...
// ── DashboardStats ────────────────────────────────────────────────────────────
/// Matched to /analytics/stats response keys.
/// health_score, qr_scans, conversionRate are derived if not present.
class DashboardStats {
  ...
  factory DashboardStats.fromJson(Map<String, dynamic> json) {
    ...
    // Derive health score from available data if backend doesn't send it
    double healthScore = _d(json['health_score']);
    if (healthScore == 0 && totalOrders > 0) {
      final convRate = paidOrders / totalOrders;
      healthScore = ((convRate * 60) + (aiHandled > 0 ? 20 : 0) + 20)
          .clamp(0.0, 100.0);
    }
  ...
  // Convenience aliases used by home/analytics screens
...
class Order {
  ...
  final String status;     // normalised to: new|preparing|completed|cancelled
  final String rawStatus;  // original backend status
  final double total;
  ...
  /// True when this order should show a "Navigate" action — i.e. it's a
  /// delivery order with an address on file.
  bool get isDelivery =>
  ...
  factory Order.fromJson(Map<String, dynamic> json) {
    ...
    return Order(
      ...
      rawStatus: rawStatus,
      // Backend uses total_price, not total
  ...
  /// Map backend statuses to the 4 UI display statuses.
...
class OrderItem {
  ...
  factory OrderItem.fromJson(Map<String, dynamic> json) => OrderItem(
        // Backend sends: {name, price, qty} or {product_name, quantity, price}
...
// ── Conversation ──────────────────────────────────────────────────────────────
/// Matches GET /chat/conversations response exactly.
class Conversation {
  final String customerId;
  final String phone;
  final String? customerName;
  ...
  final String? lastDirection; // 'incoming' | 'outgoing'
...
// ── Message ───────────────────────────────────────────────────────────────────
/// Matches GET /chat/messages/:customer_id response.
/// Backend field is 'text' (not 'content'), direction is 'incoming'/'outgoing'.
class Message {
  final String id;
  final String direction;   // 'incoming' | 'outgoing'
  final String content;     // mapped from backend 'text'
  final String? type;
  final String? senderType; // 'ai' | 'agent' | 'customer'
  final bool isRead;
  final String createdAt;
